...
public enum CameraDevice:String {
    case Front = "Front"
    case Back = "Back"
}
...
public class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate {
    ...
    private var library: CustomPhotoAlbum?
    ...
    */
                            ...
                            if let weakSelf = self {
                                if weakSelf.writeFilesToPhoneLibrary {
                                    if let validLibrary = weakSelf.library {
                                        validLibrary.saveImage(image!, metadata:["DatePhotoTaken":NSDate()])
                                    }
                                }
                            }
        ...
        if (error != nil) {
            ...
            //if let validLibrary = library {
                if writeFilesToPhoneLibrary {
                    ...
                    delegate!.cameraManagerDidGenerateVideoAsset(outputFileURL)
                    //validLibrary.saveVideo(outputFileURL, metadata:["DateVideoTaken":NSDate()])
    ...
    private func _startFollowingDeviceOrientation() {
        if shouldRespondToOrientationChanges && !cameraIsObservingDeviceOrientation {
            NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CameraManager.orientationChanged), name: UIDeviceOrientationDidChangeNotification, object: nil)
